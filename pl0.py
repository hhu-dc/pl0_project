...
class Ir(NamedTuple):
    op    : IrOpCode
    args  : str or int or None        = None
    value : int or list['Ir'] or None = None


# the reason why need to difine ctx is that we need to aviod the redefination
class EvalContext(NamedTuple):
    vars   : dict[str, int or None]
    procs  : dict[str, 'Procedure']
    consts : dict[str, int]

class Factor(NamedTuple):
    value: str or int or 'Expression'
    ...
    def eval(self, ctx: EvalContext) -> int or None:
        if isinstance(self.value, int):
            return self.value
...
class Call(NamedTuple):
    ...
    def eval(self, ctx: EvalContext) -> int or None:
        if self.name not in ctx.procs:
            raise RuntimeError('undefined procedure: ' + self.name)
...
class Condition(NamedTuple):
    cond: OddCondition or StdCondition
...
class Statement(NamedTuple):
    stmt: Assign or Call or Begin or If or While or InputOutput
...
class Parser:
    ...
    def check(self, ty: TokenKind, val: str or int) -> bool:
        p = self.lx.i
        tk = self.lx.next()
    ...
    def expect(self, ty: TokenKind, val: str or int or None = None):
        tk = self.lx.next()
...
def ir_eval(buf: list[Ir], ctx: EvalContext):
    ...
    while pc < len(buf):
        ...
        if ir.op == IrOpCode.Add:
            ...
            if not isinstance(ir.args, str) or not isinstance(ir.value, Procedure):
